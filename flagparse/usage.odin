...
@(private)
print_usage_exit :: proc(code: int) {
    print_usage();
    os.exit(code);
}

@(private)
print_exit :: proc(code: int, format: string, args: ..any) {
    fmt.eprintf(format, ..args);
    os.exit(code);
}

@(private)
print_flag :: proc(key_chr: u8, key_str, desc: string, space_count: int) {
...
print_usage :: proc() {
    ...
    print_flag('h', "help", "print usage", space_count);

    for i := 0; i < len(FLAG_ARRAY); i += 1 {
        space_count = FLAGSTR_MAX - len(FLAG_ARRAY[i].flag_str);
        print_flag(FLAG_ARRAY[i].flag_char, FLAG_ARRAY[i].flag_str, FLAG_ARRAY[i].desc, space_count);
    }
}